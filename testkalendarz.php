...
// Funkcja pomocnicza do uzyskania numeru tygodnia w roku na podstawie daty
function getWeekNumber($date) {
    return date('W', strtotime($date)); 
}

// Wprowadzenie do sprawdzania danych i przypisania właściwego koloru
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit-hours'])) {
    ...
    $week_number = getWeekNumber($start); // Uzyskujemy numer tygodnia
    ...
    if ($_POST['hourlyOrWeekly'] === 'weekly') {
        ...
        if ($totalHours >= 40) {
            echo "<script>alert('Wprowadzono już 40h pracy na ten tydzień. Nie można dodać więcej.'); window.location.href='testkalendarz.php';</script>";
            exit;
        }
        ...
        header("Location: testkalendarz.php");
        exit;
    ...
    <div class="container">
        <div class="sidebar">
            <ul class="sidebar-menu">
                ...
                <li><a href="testkalendarz.php">Kalendarz</a></li>
                <?php
            // Sprawdzamy, czy użytkownik ma rolę 'admin'
            if (isset($_SESSION['role']) && $_SESSION['role'] == 'admin') {
            // Jeśli rola to 'admin', wyświetlamy link do strony administratora
...
<div id="workModal" class="modal">
    <div class="modal-content">
        ...
        <form method="post" action="testkalendarz.php">
...
function changeYear(offset) {
    ...
    if (newYear >= 2020 && newYear <= 2030) {
        window.location.href = `testkalendarz.php?year=${newYear}`;
    }
}
